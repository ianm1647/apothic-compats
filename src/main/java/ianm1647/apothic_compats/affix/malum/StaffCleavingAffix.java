...
public class StaffCleavingAffix extends Affix {
    public static final Codec<StaffCleavingAffix> CODEC = RecordCodecBuilder.create((inst) -> {
        return inst.group(affixDef(), LootRarity.mapCodec(StaffCleavingAffix.CleaveValues.CODEC).fieldOf("values").forGetter((a) -> {
            return a.values;
        })).apply(inst, StaffCleavingAffix::new);
    });
    protected final Map<LootRarity, StaffCleavingAffix.CleaveValues> values;
    private static boolean cleaving = false;

    public StaffCleavingAffix(AffixDefinition def, Map<LootRarity, StaffCleavingAffix.CleaveValues> values) {
        super(def);
        this.values = values;
    }
    ...
    private float getChance(LootRarity rarity, float level) {
        return ((StaffCleavingAffix.CleaveValues)this.values.get(rarity)).chance.get(level);
    }

    private int getTargets(LootRarity rarity, float level) {
        return ((StaffCleavingAffix.CleaveValues)this.values.get(rarity)).targets.getInt(level);
    }
    ...
    static record CleaveValues(StepFunction chance, StepFunction targets) {
        public static final Codec<StaffCleavingAffix.CleaveValues> CODEC = RecordCodecBuilder.create((inst) -> {
            return inst.group(StepFunction.CODEC.fieldOf("chance").forGetter((c) -> {
                return c.chance;
            }), StepFunction.CODEC.fieldOf("targets").forGetter((c) -> {
                return c.targets;
            })).apply(inst, StaffCleavingAffix.CleaveValues::new);
        });
    ...
    public static class Builder extends AffixBuilder<StaffCleavingAffix.Builder> {
        protected final Map<LootRarity, StaffCleavingAffix.CleaveValues> values = new HashMap();

        public Builder() {
        }

        public StaffCleavingAffix.Builder value(LootRarity rarity, float minChance, float maxChance, int minTargets, int maxTargets) {
            ...
            this.values.put(rarity, new StaffCleavingAffix.CleaveValues(chance, targets));
            return this;
        }

        public StaffCleavingAffix build() {
            Preconditions.checkNotNull(this.definition);
            Preconditions.checkArgument(this.values.size() > 0);
            return new StaffCleavingAffix(this.definition, this.values);
        }
    }
}
