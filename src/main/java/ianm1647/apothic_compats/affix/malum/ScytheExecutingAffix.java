...
public class ScytheExecutingAffix extends Affix {

    public static final Codec<ScytheExecutingAffix> CODEC = RecordCodecBuilder.create(inst -> inst
            .group(
                    affixDef(),
                    LootRarity.mapCodec(StepFunction.CODEC).fieldOf("values").forGetter(a -> a.values))
            .apply(inst, ScytheExecutingAffix::new));

    protected final Map<LootRarity, StepFunction> values;

    public ScytheExecutingAffix(AffixDefinition def, Map<LootRarity, StepFunction> values) {
        super(def);
        this.values = values;
    }
